...
- **Visual thesis:** an annotated working field manual on warm paper: black
  rules, serif display type, plain sans-serif controls, vermilion corrections,
  and one acid-yellow signal used only for orientation.
...
- **Reference source:** the Humanizer ChatGPT Site supplies paper, ink,
  ...
  Defense Field Guide supplies hard borders, numbered navigation, highlighter
  color, and task-first sections.
...
- **Trust:** 9/10. Public download, connected service, preview, and local-only
  states are explicit. Generated art never sits in a proof slot.
...
- No claim that a local MCP or connector is a public install.
...
| R2 | Participatory Defense Field Guide | Hero safety rail and numbered sections | Hard rules and highlighter signal create urgency without visual effects | reference | Reuse border logic and signal color sparingly | Design Taste Gate |
| N1 | New Working Tools hero | Right-side generated collage | Needs an authored visual anchor without pretending to show real product output | atmosphere | Generate a text-free editorial collage; constrain crop and height | asset manifest plus desktop/mobile screenshots |
